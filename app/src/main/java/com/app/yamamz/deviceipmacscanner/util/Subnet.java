...
//	/** stores the Network Increment. */
//	private int net_increment;
    ...
    public void setTotalSubnets(int totalSubnets) {
        total_subnets = Conversion.numRound(totalSubnets);
        int subnetBits = (int)(Math.log(total_subnets) / Math.log(2)); // 2^x = totalSubnets; x = log2(totalSubnets)
    ...
    public void setTotalHosts(int totalHosts) {
        total_hosts = Conversion.numRound(totalHosts);
        int hostBits = (int)(Math.log(total_hosts) / Math.log(2));
